...
def expected_calibration_error(y_true, y_prob, n_bins=10):
    ...
    # Create bin edges: 0, 1/n_bins, 2/n_bins, ..., 1
    ...
    for i in range(n_bins):
        ...
        if i == 0:
            # First bin: [0, 1/n_bins] - inclusive on both ends
            mask = (y_prob >= lower) & (y_prob <= upper)
        else:
            # Other bins: (lower, upper] - exclusive lower, inclusive upper
            mask = (y_prob > lower) & (y_prob <= upper)
        
        # Get number of samples in this bin
        ...
        # Calculate weight: proportion of samples in bin
